...
void QMPServer::setup_socket() {
    ...
    // which previously left the socket unbound and spun accept() forever.
    if (setsockopt(server_fd, SOL_SOCKET, SO_REUSEADDR, &opt, sizeof(opt)) < 0) {
        LOG(LOG_REMOTE, LOG_ERROR)("QMP: setsockopt(SO_REUSEADDR) failed: %s", strerror(errno));
        close(server_fd);
        server_fd = -1;
        return;
    }
    ...
    if (setsockopt(server_fd, SOL_SOCKET, SO_REUSEPORT, &opt, sizeof(opt)) < 0) {
        LOG(LOG_REMOTE, LOG_WARN)("QMP: setsockopt(SO_REUSEPORT) failed: %s", strerror(errno));
    }
#endif
